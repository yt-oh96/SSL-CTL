...
parser.add_argument('--exp', type=str,
                    default='UFMR_DCMR/Uncertainty_Aware_Mean_Teacher', help='experiment_name')
    ...
    for epoch_num in iterator:
        for i_batch, (sampled_batch, sampled_batch2) in enumerate(trainloader):
            ...
            if iter_num % 20 == 0:
                ...
                sitk.WriteImage(source, os.path.join('./entropy_minmization', '{}_image.nii.gz'.format(iter_num)))
        
                o = sitk.GetImageFromArray(outputs.data.cpu().float())
                sitk.WriteImage(o, os.path.join('./entropy_minmization', '{}_o.nii.gz'.format(iter_num)))
                
                l = sitk.GetImageFromArray(labs.data.cpu())
                sitk.WriteImage(l, os.path.join('./entropy_minmization', '{}_l.nii.gz'.format(iter_num)))
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                if combine > best_performance_combine:
                    ...
                    save_best = os.path.join(snapshot_path,
                                             '{}_best_model_combine.pth'.format(args.model))
