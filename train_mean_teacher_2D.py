...
parser.add_argument('--exp', type=str,
                    default='UFMR_DCMR/Mean_Teacher', help='experiment_name')
    ...
    for epoch_num in iterator:
        for i_batch, (sampled_batch, sampled_batch2) in enumerate(trainloader):
            ...
            if iter_num % 20 == 0:
                ...
                sitk.WriteImage(source, os.path.join('./mean_teacher', '{}_image.nii.gz'.format(iter_num)))
        
                o = sitk.GetImageFromArray(outputs.data.cpu().float())
                sitk.WriteImage(o, os.path.join('./mean_teacher', '{}_o.nii.gz'.format(iter_num)))
                
                l = sitk.GetImageFromArray(labs.data.cpu())
                sitk.WriteImage(l, os.path.join('./mean_teacher', '{}_l.nii.gz'.format(iter_num)))
