...
#     writer = SummaryWriter(snapshot_path + '/log')
    ...
    for epoch_num in iterator:
        for i_batch, (sampled_batch, sampled_batch2) in enumerate(trainloader):
            ...
            outputs2 = model2((tdm_batch-1.0)) #version_2
            outputs_soft2 = torch.softmax(outputs2, dim=1)
...
#                               model2_loss, iter_num)
            ...
            if iter_num % 50 == 0:
                ...
                sitk.WriteImage(source, os.path.join('./tdm_cross_pseudo_v2', '{}_image.nii.gz'.format(iter_num)))
        
                o = sitk.GetImageFromArray(outputs.data.cpu().float())
                sitk.WriteImage(o, os.path.join('./tdm_cross_pseudo_v2', '{}_o.nii.gz'.format(iter_num)))
                
                o2 = sitk.GetImageFromArray(outputs2.data.cpu().float())
                sitk.WriteImage(o2, os.path.join('./tdm_cross_pseudo_v2', '{}_o2.nii.gz'.format(iter_num)))
                
                l = sitk.GetImageFromArray(labs.data.cpu())
                sitk.WriteImage(l, os.path.join('./tdm_cross_pseudo_v2', '{}_l.nii.gz'.format(iter_num)))
...
if __name__ == "__main__":
    ...
    wandb.init(project='SSL_UFMR_DCMR_TDM_cross_pseudo_test')
    wandb.run.name = 'SSL_UFMR_DCMR_TDM_cross_pseudo_v2'
