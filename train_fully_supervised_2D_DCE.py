...
from dataloaders.dataset_dce import BaseDataSets, RandomGenerator
...
parser.add_argument('--exp', type=str,
                    default='UFMR_DCMR/Fully_Supervised', help='experiment_name')
parser.add_argument('--model', type=str,
                    default='unet', help='model_name')
parser.add_argument('--num_classes', type=int,  default=2,
                    help='output channel of network')
parser.add_argument('--max_iterations', type=int,
                    default=7000, help='maximum epoch number to train')
...
#             writer.add_scalar('info/loss_dice', loss_dice, iter_num)
            ...
            if iter_num % 20 == 0:
                ...
                sitk.WriteImage(source, os.path.join('./test', '{}_image.nii.gz'.format(iter_num)))
        
                o = sitk.GetImageFromArray(outputs.data.cpu().float())
                sitk.WriteImage(o, os.path.join('./test', '{}_o.nii.gz'.format(iter_num)))
                
                l = sitk.GetImageFromArray(labs.data.cpu())
                sitk.WriteImage(l, os.path.join('./test', '{}_l.nii.gz'.format(iter_num)))
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for class_i in range(num_classes-1):
                    wandb.log({
                        "v_iter_num" : iter_num,
                        "val_{}_dice".format(class_i+1) : metric_list[class_i, 0],
                        "val_{}_hd95".format(class_i+1) : metric_list[class_i, 1],
                    })
...
#                 writer.add_scalar('info/val_mean_hd95', mean_hd95, iter_num)
                combine = (performance*10.0-mean_hd95+5)/10
