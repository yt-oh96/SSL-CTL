...
def Inference(FLAGS, mode):
    ...
    if mode == 'best':
        ...
        save_mode_path_combine = os.path.join(
            snapshot_path, '{}_best_model_combine.pth'.format(FLAGS.model))
    elif mode == 'last':
        save_mode_path_last = os.path.join(
            snapshot_path, 'model_iter_15000.pth'.format(FLAGS.model))
