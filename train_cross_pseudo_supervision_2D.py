...
parser.add_argument('--root_path', type=str,
                    default='/notebook/SSL4MIS/data/UFMR_DCMR', help='N')
...
#                               model2_loss, iter_num)
            ...
            if iter_num % 50 == 0:
                ...
                sitk.WriteImage(source, os.path.join('./cross_pseudo', '{}_image.nii.gz'.format(iter_num)))
        
                o = sitk.GetImageFromArray(outputs.data.cpu().float())
                sitk.WriteImage(o, os.path.join('./cross_pseudo', '{}_o.nii.gz'.format(iter_num)))
                
                o2 = sitk.GetImageFromArray(outputs2.data.cpu().float())
                sitk.WriteImage(o2, os.path.join('./cross_pseudo', '{}_o2.nii.gz'.format(iter_num)))
                
                l = sitk.GetImageFromArray(labs.data.cpu())
                sitk.WriteImage(l, os.path.join('./cross_pseudo', '{}_l.nii.gz'.format(iter_num)))
